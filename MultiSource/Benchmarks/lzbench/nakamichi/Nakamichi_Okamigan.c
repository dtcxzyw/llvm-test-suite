...
NumberOfFullLiterals++;
			...
			if ((srcIndex-1) % (1<<16) > srcIndex % (1<<16)) {
				...
				//printf("%s; Each rotation means 64KB are encoded; Speed: %sB/s; Done %d%%; Compression Ratio: %.2f:1; Matches(24/32/48): %s/%s/%s    \n", Auberge[Melnitchka++], _ui64toaKAZEzerocomma(SPD, llTOaDigits4, 10)+(26-7) ,ProgressIndicator, (double)(srcIndex) / (double)(retIndex),  _ui64toaKAZEcomma(GLOBAL24B, llTOaDigits2, 10), _ui64toaKAZEcomma(GLOBAL32B, llTOaDigits, 10), _ui64toaKAZEcomma(GLOBAL48B, llTOaDigits3, 10) );
