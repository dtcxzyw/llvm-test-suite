...
void lzbench_test(lzbench_params_t *params, std::vector<size_t> &file_sizes, const compressor_desc_t* desc, int level, uint8_t *inbuf, size_t insize, uint8_t *compbuf, size_t comprsize, uint8_t *decomp, bench_rate_t rate, size_t param1)
{
    ...
    do
    {
        ...
        //LZBENCH_PRINT(2, "%s compr iter=%d time=%.2fs speed=%.2f MB/s     \n", desc->name, total_c_iters, total_nanosec/1000000000.0, speed);
    }
    while (true);
    ...
    do
    {
        ...
        //LZBENCH_PRINT(2, "%s decompr iter=%d time=%.2fs speed=%.2f MB/s     \n", desc->name, total_d_iters, total_nanosec/1000000000.0, (float)insize*i*1000/nanosec);
    }
    while (true);
