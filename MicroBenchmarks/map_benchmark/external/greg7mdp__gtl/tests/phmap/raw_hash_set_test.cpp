...
ExpectedStats XorSeedExpectedStats() {
  ...
  //GTL_RAW_LOG(FATAL, "%s", "Unknown Group width");
  return {};
}
...
TEST(Table, NoThrowMoveConstruct) {
  ASSERT_TRUE(
      std::is_nothrow_copy_constructible<gtl::Hash<std::string_view>>::value);
...
TEST(Table, NoThrowMoveAssign) {
  ASSERT_TRUE(
      std::is_nothrow_move_assignable<gtl::Hash<std::string_view>>::value);
  ASSERT_TRUE(
...
TEST(Table, HeterogeneousLookupOverloads) {
  using NonTransparentTable =
      raw_hash_set<StringPolicy, gtl::Hash<std::string_view>,
                   std::equal_to<std::string_view>, std::allocator<int>>;
  ...
  using TransparentTable = raw_hash_set<
      StringPolicy,
      gtl::priv::hash_default_hash<std::string_view>,
      gtl::priv::hash_default_eq<std::string_view>,
      std::allocator<int>>;
