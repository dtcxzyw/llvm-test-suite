...
void GameOfLife_stabilizing(benchmark::State &state) {
    ...
    for(auto _ : state) {
        ...
        game_of_life("R-pentomino", 1103, 116, map1, {{1, 0}, {2, 0}, {0, 1}, {1, 1}, {1, 2}});

        // https://conwaylife.com/wiki/Acorn
        game_of_life("Acorn", 5206, 633, map1, {{1, 0}, {3, 1}, {0, 2}, {1, 2}, {4, 2}, {5, 2}, {6, 2}});

        // https://conwaylife.com/wiki/Jaydot
        game_of_life("Jaydot", 6929, 1124, map1, {{1, 0}, {2, 0}, {0, 1}, {1, 1}, {2, 1}, {1, 3}, {1, 4}, {2, 4}, {0, 5}});

        // https://conwaylife.com/wiki/Bunnies
        game_of_life("Bunnies", 17332, 1744, map1, {{0, 0}, {6, 0}, {2, 1}, {6, 1}, {2, 2}, {5, 2}, {7, 2}, {1, 3}, {3, 3}});
    }
}

//BENCHMARK(GameOfLife_stabilizing);

void GameOfLife_growing(benchmark::State &state) {
    using M = Map<uint32_t, bool>;
#ifdef USE_POOL_ALLOCATOR
    Resource<uint32_t, bool> resource;
    M map1{0, M::hasher{}, M::key_equal{}, &resource};
